...
class ImageResize1(object):
    ...
    def __call__(self, rendering_images, bounding_box=None):
        ...
        size_c = rendering_images[0].shape[2]
        processed_images = np.empty(shape=(0, self.img_size_h, self.img_size_w, size_c))
        for img_idx, img in enumerate(rendering_images):
            img_height, img_width, _ = img.shape
            print('[INFO] %s Scaling images to %sx%s' % (dt.now(), cfg.CONST.IMG_W, cfg.CONST.IMG_H))
            ...
            if img_height != cfg.CONST.IMG_H or img_width != cfg.CONST.IMG_W:

                print('[INFO] %s Scaling images to %sx%s' % (dt.now(), cfg.CONST.IMG_W, cfg.CONST.IMG_H))

                tmp = np.zeros((cfg.CONST.IMG_H, cfg.CONST.IMG_W, size_c), np.float)
                if (img_height > img_width):
                    ratio = img_width / img_height
                    w = int(ratio * cfg.CONST.IMG_H)
                    h = cfg.CONST.IMG_H
                else:
                    ratio = img_height / img_width
                    w = cfg.CONST.IMG_W
                    h = int(ratio * cfg.CONST.IMG_W)
                image = cv2.resize(img[0:img_height, 0:img_width], (w, h))
                tmp[0:h, 0:w] = image
                #cv2.imshow("image", tmp)
                #cv2.waitKey(0)

            processed_images = np.append(processed_images, [tmp], axis=0)

            fig = plt.figure()
            ax1 = fig.add_subplot(1, 2, 1)
            ax1.imshow(img)
            if not bounding_box is None:
                rect = patches.Rectangle((bounding_box[0], bounding_box[1]), bbox_width, bbox_height, linewidth=1,
...
class ImageResize(object):
    ...
    def __call__(self, rendering_images, bounding_box=None):
        ...
        for img_idx, img in enumerate(rendering_images):
            img_height, img_width, _ = img.shape
            print('[INFO] %s Scaling images to %sx%s' % (dt.now(), cfg.CONST.IMG_W, cfg.CONST.IMG_H))
            image = cv2.resize(img, (self.img_size_w, self.img_size_h))
            processed_images = np.append(processed_images, [image], axis=0)
