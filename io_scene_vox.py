...
def import_vox(path, *, voxel_spacing=1, voxel_size=1, load_frame=0,
    ...
    #os.system("cls")
    print("\n")
    print("[INFO] %s Importing voxel file from: %s" % (dt.now(), path))
    ...
    print("[INFO] %s Loaded voxels: %i" % (dt.now(), len(voxels)))
    ...
    print("[INFO] %s Linked voxels: %i" % (dt.now(),len(to_link)))
    ...
    print("[INFO] %s Successfully imported the vox file in %.2f seconds" % (dt.now(), time.time() - time_start))
    print("\n")
    return {'FINISHED'}
